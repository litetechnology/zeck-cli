...
const setVps = async (tools, config) => {
    ...
    config.content.profiles[config.content.currentProfile] = {...answers, ...config.content.profiles[config.content.currentProfile]};
...
const setUrl = async (tools, config) => {
    ...
    config.content.profiles[config.content.currentProfile] = {url: answers[0], ...config.content.profiles[config.content.currentProfile]};
    config.update({profiles: config.content.profiles});
    return { status: true, data: answers }
};
